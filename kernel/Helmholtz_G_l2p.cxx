...
extern void multipoleh(double, double, double, int);
extern void in(double, double, const int, double*, int);

void Helmholtz_G_l2p(int nmp, int mp, int lbi) {
  int kl,ii,i,n,nm,nms,m,nc[3];
  double rb,xic,yic,zic,xiic,yiic,ziic,r,th,ph,g,gx,gy,gz,gr,gth,gph;
  double inr[mpcmp];
  ...
  for( ii=0; ii<lbi; ii++ ) {
    ...
    for( i=ndi[0][ii]; i<=ndi[1][ii]; i++ ) {
      ...
      gth = 0;
      gph = 0;
      if(scale == 0.1) scale *= rb;
      multipoleh(r,th,ph,mp);
      for( n=0; n<mp; n++ ) {
        for( m=0; m<=n; m++ ) {
          nm = n*n+n+m;
          bnm[nm] /= (2*n+1)*fac[nm]*fac[nm];
        }
      }
      for( n=0; n<mp; n++ ) {
        nm = n*n+n;
