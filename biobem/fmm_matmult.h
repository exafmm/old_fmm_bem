...
// Stage 1. Prepare source data for far field
...
// Stage 2. Prepare source data for principal integral
  nj = 0;
  // Run over source elements
...
PetscErrorCode mypcshell(PC pc,Vec X,Vec Y)
{
  ...
  for(PetscInt l = 0; l < nwn; ++l) y[l] = x[l]/(e1+e2)*(2*e1*(e1-e2));
//  for(PetscInt l = 0; l < nwn; ++l) y[l] = x[l];
